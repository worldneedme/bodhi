            ...
            for line_stat in (curl -sL "http://127.0.0.1:$api_port/metrics")
                if string match -q '*'\#'*' -- $line_stat; or string match -q '*active_conn*' -- $line_stat
                    ...
                    if string match -rq uplink -- $line_stat
                        ...
                        set upload[$line_id] (math (string split ' ' -- $line_stat)[2] - $last_upload[$line_id])
                        set last_upload[$line_id] (string split ' ' -- $line_stat)[2]
                    else
                        if test -z last_download[$line_id]
                            set last_download[$line_id] 0
                        end
                        set download[$line_id] (math (string split ' ' -- $line_stat)[2] - $last_download[$line_id])
                        set last_download[$line_id] (string split ' ' -- $line_stat)[2]
                    end
                end
            end
            # compose json data
